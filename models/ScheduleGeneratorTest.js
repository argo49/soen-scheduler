...
GenerateSchedules(['COMP 232', 'COMP 248', 'ENGR 201', 'ENGR 213'], 2, null, function (error, data) {
	mergeSimilar(data);
});

function GenerateSchedules (CourseList, Session, Preferences, Callback) {
...
function mergeSimilar (Schedules) {
	...
	for (var a = 0; a < Schedules.length; a++) {
		for (var b = a+1; b <  Schedules.length; b++) {
			...
			console.log('\t'+Schedules[a].Id);
			console.log('\t'+Schedules[b].Id);
			similarSchedule(Schedules[a], Schedules[b]);
		}
	}
}
...
function similarSchedule (schedA, schedB) {
	...
	for (var a = 0; a < schedA.length; a++) {
		for (var b = 0; b < schedB.length; b++) {
			if (schedA[a].Course == schedB[b].Course) continue;
		}
	}
